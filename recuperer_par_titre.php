...
if (isset($_GET["titre"])) {
    ...
    $query = $con->prepare("SELECT id, titre, description, langue, genre, date_de_sortie, box_offixe, duree, nombre_etoile FROM films WHERE titre = ?");
    
    if ($query === false) {
        die("Erreur de préparation de la requête : " . $con->error);
    }

    $query->bind_param("s", $titre); // Utilisez "s" pour le type de chaîne (string)
    ...
    $response['error'] = true;
    $response['message'] = 'Veuillez fournir un titre de film à rechercher';
}
